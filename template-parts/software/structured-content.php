...
/**
 * Choose a balanced desktop column count based on card count.
 *
 * Up to four cards use one column per card. Five and six cards
 * use three columns, while seven and eight cards use four columns.
 *
 * @param array $items Section rows.
 * @return int
 */
$get_product_grid_columns = static function ( $items ) {
    if ( ! is_array( $items ) ) {
        return 1;
    }
...
$benefit_columns = $get_product_grid_columns( $benefits );
$addon_columns   = $get_product_grid_columns( $addons );
$package_columns = $get_product_grid_columns( $packages );
if (
    empty( $benefits )
    && empty( $features )
    && empty( $addons )
...
// ETOS SOFTWARE FEATURE FALLBACK SETTINGS
// Tymczasowy wariant prezentacyjny dla klienta.
$feature_fallback_enabled = false;

$feature_fallback_items = array(
    array(
        'title' => 'Obsługa sprzedaży i zamówień',
        'text'  => 'Prowadź cały proces sprzedaży w jednym miejscu — od zamówienia i rezerwacji towaru po dokumenty sprzedażowe i realizację.',
    ),
    array(
        'title' => 'Fakturowanie i dokumenty handlowe',
        'text'  => 'Wystawiaj faktury, korekty, paragony oraz pozostałe dokumenty związane z codzienną obsługą sprzedaży.',
    ),
    array(
        'title' => 'Gospodarka magazynowa',
        'text'  => 'Kontroluj stany magazynowe, przyjęcia, wydania i przesunięcia oraz zachowuj pełną informację o przepływie towarów.',
    ),
    array(
        'title' => 'Kontrahenci i historia współpracy',
        'text'  => 'Przechowuj dane klientów i dostawców wraz z dokumentami, rozrachunkami oraz historią realizowanych transakcji.',
    ),
    array(
        'title' => 'Rozrachunki i płatności',
        'text'  => 'Kontroluj należności i zobowiązania, terminy płatności oraz bieżący stan rozliczeń z kontrahentami.',
    ),
    array(
        'title' => 'Cenniki, rabaty i warunki handlowe',
        'text'  => 'Buduj elastyczne cenniki i zasady rabatowe dopasowane do klientów, grup odbiorców i prowadzonej polityki sprzedażowej.',
    ),
    array(
        'title' => 'Raporty i analiza danych',
        'text'  => 'Korzystaj z bieżących informacji o sprzedaży, marży, magazynie i płatnościach, aby szybciej podejmować decyzje.',
    ),
    array(
        'title' => 'Integracja procesów w firmie',
        'text'  => 'Łącz sprzedaż i magazyn z księgowością, e-commerce oraz innymi obszarami systemu, ograniczając ręczne przenoszenie danych.',
    ),
);

if ( $feature_fallback_enabled ) {
    $features = array();
}
$post_id = get_the_ID();
...
<div class="etos-product-sections">
    ...
    <?php if ( ! empty( $benefits ) ) : ?>
        ...
        >
            ...
            <div
                class="etos-product-grid etos-product-grid--benefits etos-product-grid--cols-<?php echo esc_attr( $benefit_columns ); ?>"
            >
    ...
    <?php if ( $feature_fallback_enabled ) : ?>

        <!-- ETOS SOFTWARE FEATURE FALLBACK START -->

        <section
            class="etos-feature-overview"
            aria-labelledby="software-feature-overview-<?php echo esc_attr( $post_id ); ?>"
        >

            <div class="etos-feature-overview__intro">

                <span class="etos-product-section__eyebrow">
                    <?php esc_html_e( 'Funkcjonalności programu', 'etos' ); ?>
                </span>

                <h2
                    class="etos-feature-overview__title"
                    id="software-feature-overview-<?php echo esc_attr( $post_id ); ?>"
                >
                    <?php esc_html_e( 'Funkcjonalności, które docenisz', 'etos' ); ?>
                </h2>

                <p class="etos-feature-overview__lead">
                    <?php
                    esc_html_e(
                        'Najważniejsze narzędzia wspierające codzienną pracę firmy — bez zbędnej komplikacji i przełączania się pomiędzy wieloma systemami.',
                        'etos'
                    );
                    ?>
                </p>
            ...
            <div class="etos-feature-overview__list">

                <?php foreach ( $feature_fallback_items as $index => $item ) : ?>

                    <article class="etos-feature-overview__item">

                        <span
                            class="etos-feature-overview__number"
                            aria-hidden="true"
                        >
                            <?php
                            echo esc_html(
                        ...
                        <h3 class="etos-feature-overview__item-title">
                            <?php echo esc_html( $item['title'] ); ?>
                        </h3>

                        <p class="etos-feature-overview__text">
        ...
        <!-- ETOS SOFTWARE FEATURE FALLBACK END -->
    ...
    <?php if ( ! empty( $addons ) ) : ?>
        ...
        >
            ...
            <div
                class="etos-product-grid etos-product-grid--addons etos-product-grid--cols-<?php echo esc_attr( $addon_columns ); ?>"
            >
    ...
    <?php if ( ! empty( $packages ) ) : ?>
        ...
        >
            ...
            <div
                class="etos-product-grid etos-product-grid--packages etos-product-grid--cols-<?php echo esc_attr( $package_columns ); ?>"
            >
