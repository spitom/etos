...
class ETOS_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
		...
		$is_software  = in_array( 'etos-menu-software', $classes, true );
		$is_services  = in_array( 'etos-menu-services', $classes, true );
		$is_cta       = in_array( 'etos-menu-cta', $classes, true );
		$is_dropdown  = $is_software || $is_services;
		...
		$atts = array(
			...
			'class'        => $is_cta ? 'nav-link etos-navbar__cta' : 'nav-link',
		);
	...
	/**
	 * Render a parent link and a separate accessible dropdown toggle.
	 *
						...
						<?php foreach ( $vendors as $vendor ) : ?>
							...
							>
								<a
									...
									href="<?php echo esc_url( is_wp_error( $vendor_link ) ? '#' : $vendor_link ); ?>"
								>
									<?php
									if ( $logo_id ) {
